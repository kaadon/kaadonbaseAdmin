...
class Payment extends BaseMerchant
{
    ...
    public function getPayment()
    {
        ...
        $PayOrder = MemberPayOrder::hasWhere('account', $wheres);
        ...
        $where = [];
        $status = $this->request->param('status/s',null);
        if (!is_null($status)){
            $where['MemberPayOrder.status'] = $status;
        }
        $PayOrder = $PayOrder->where($where);

        $count = $PayOrder->count();
        $PayOrder = $PayOrder->page($page)
            ->limit($limit)->order('MemberPayOrder.id desc')->select();
